...
// Includes subscription_status/grandfathered too, since api/world.js also
// reuses this list to check whether a user already has a paying world
// before letting them start another free-preview one.
export async function getWorldsForUser(userId) {
  return sbFetch(
    '/worlds?user_id=eq.' + userId
    + '&order=last_visited_at.desc'
    + '&select=id,title,genre,chapter_cap,world_hours,last_visited_at,created_at,subscription_status,grandfathered'
  );
}
...
// Generic partial-update, undefined-stripped, for fields that aren't
// billing-specific (e.g. resetting a world's clock and metadata on restart).
export async function updateWorldFields(worldId, fields) {
  await sbFetch('/worlds?id=eq.' + worldId, {
    method: 'PATCH',
    headers: { 'Prefer': 'return=minimal' },
...
// Wipes everything a world's story is made of -- used when restarting a
// subscribed world into a brand-new story. Never touches the worlds row
// itself (title/genre/tone/clock are reset separately), so billing fields
// are untouched no matter what.
export async function deleteWorldContent(worldId) {
  await Promise.all([
    sbFetch('/characters?world_id=eq.' + worldId, { method: 'DELETE', headers: { 'Prefer': 'return=minimal' } }),
    sbFetch('/locations?world_id=eq.' + worldId, { method: 'DELETE', headers: { 'Prefer': 'return=minimal' } }),
    sbFetch('/events?world_id=eq.' + worldId, { method: 'DELETE', headers: { 'Prefer': 'return=minimal' } }),
    sbFetch('/user_character?world_id=eq.' + worldId, { method: 'DELETE', headers: { 'Prefer': 'return=minimal' } }),
    sbFetch('/story_state?world_id=eq.' + worldId, { method: 'DELETE', headers: { 'Prefer': 'return=minimal' } }),
  ]);
}
...
export async function setFreePreviewTurnsUsed(worldId, count) {
  await sbFetch('/worlds?id=eq.' + worldId, {
    method: 'PATCH',
    headers: { 'Prefer': 'return=minimal' },
    body: JSON.stringify({ free_preview_turns_used: count }),
  });
}
