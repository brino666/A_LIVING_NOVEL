...
- When is_last_chapter_of_day is true, this is the reader's last chapter
  until real time passes -- steer scene_focus toward closing out the day's
  throughline (a scene winding down, a return home, sleep, a door closing)
  rather than opening a new source of suspense. Don't leave the reader
  mid-anticipation on a beat they can't act on again for hours.
- world_time is the exact in-story clock this scene takes place at (e.g.
  "Day 4, 14:00" is midday, "Day 4, 02:00" is the middle of the night).
  scene_focus and npc_actions must be plausible for that specific hour --
  don't put characters at lunch, at work, or wide awake and out at a clock
  time that contradicts it.`;

function buildWorldContext(snapshot, hoursAdvanced, userAction, isLastChapterOfDay) {
  ...
  return JSON.stringify({
    ...
    is_last_chapter_of_day: !!isLastChapterOfDay,
    genre: world.genre,
...
export async function runStoryDirector({ snapshot, userAction, hoursAdvanced, isLastChapterOfDay }) {
  const context = buildWorldContext(snapshot, hoursAdvanced, userAction, isLastChapterOfDay);

  async function callDirector() {
    const response = await anthropic.messages.create({
      model: 'claude-sonnet-5',
      // A busy, long-running world (many characters/locations/events in
      // context) can push the structured JSON response past 2000 tokens on
      // Sonnet 5's denser tokenizer -- if that response gets cut off
      // mid-array, it looks identical to a malformed one downstream.
      max_tokens: 4000,
    ...
    if (response.stop_reason === 'max_tokens') {
      console.error('[novel-engine/director] response hit max_tokens -- output is likely truncated');
    }
