...
export async function writeScene({ snapshot, directorOutput, userAction }) {
  ...
  const response = await client.chat.completions.create({
    model: "grok-4.3",
    messages: [
      { role: "system", content: buildWriterSystemPrompt(world.genre, world.tone_settings, userCharacter.name) },
      { role: "user", content: 'Write this scene from the following director briefing:\n' + briefing }
    ],
    temperature: 0.85,
    max_tokens: 4000,
  });
