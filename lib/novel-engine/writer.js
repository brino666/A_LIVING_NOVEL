...
either a natural continuation moment or a soft decision point, written as
prose -- never a hard stop, never a literal game-style prompt like "What do
you do?".`;
}
...
export async function writeScene({ snapshot, directorOutput, userAction }) {
  ...
  const response = await anthropic.messages.create({
    ...
    // Sonnet 5's tokenizer runs ~30% denser than older Claude models, so 2200-2500
    // words of prose can approach 4096 tokens on its own -- this leaves real headroom
    // instead of risking a chapter truncating mid-sentence.
    max_tokens: 6000,
